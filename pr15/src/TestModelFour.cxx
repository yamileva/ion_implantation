...
void calculate(int angleLim, int angleTempLim, int numCells, double angleVert, 
  double angleAxis, double vel, double velIon,
  ...
  vtkTransformPolyDataFilter* transformFilter, vtkTransform* regTransform)
{
  ...
  vtkIdType cellId = -1;
  int k, kTemp, kSave;
  double deltaT = angleVert / vel / 360;
...
int main(int argc, char* argv[])
{
  ...
  double angleAxis,    /// угол поворота относительно оси диска
    ...
    velIon = 2 / 2.5 / 60;  /// доза облучения на 1 см^2 в минуту
  int videoAngleV,
    videoAngleA;
  ...
  if (!isCalculated)
  {
    ...
    cin >> videoAngleA;
    cout << "Введите скорость - количество оборотов вокруг вертикальной оси в минуту: ";
    cin >> vel;
  ...
  if (!isCalculated)
  {
    calculate(angleLim, angleTempLim, numCells, angleVert, angleAxis, vel, velIon,
